...
export default {
    save(file: any): Promise<{ Location: string | undefined; type: string }> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let hashFilename = await randomBytesPromisify(16).then(
                    hash => `${hash.toString('hex')}-${filename}`,
                );
                const fileBuffer: Buffer = await file.toBuffer();

                if (process.env.STORAGE_TYPE === 's3') {
                    const result: any = await s3
                        .upload({
                            Key: hashFilename,
                    ...
                    await pump(
                        readable,
                        fs.createWriteStream(`./uploads/${hashFilename}`),
                    );
                    resolve({
                        Location: `${process.env.UPLOADS_URL}/${hashFilename}`,
                        type,
                    });
                }
            } catch (error) {
    ...
    parseBody(body: { [key: string]: any }): { [key: string]: any } {
        if (!body) return {};
