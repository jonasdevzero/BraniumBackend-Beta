...
export default {
    ...
    async create(req: ServerRequest, reply: ServerReply) {
        try {
            ...
            if (!text && !medias)
                return reply
                    .status(400)
                    .send({ message: 'Você deve enviar algum dado!' });
            ...
            if (sender.blocked)
                return reply
                    .status(400)
                    .send({ message: 'Você foi bloqueado!' });

            if (sender.you_blocked)
                return reply
                    .status(400)
                    .send({ message: 'Você bloqueou este contato!' });
    ...
    async view(req: ServerRequest, reply: ServerReply) {
        try {
            ...
            const cMessageRepository = getRepository(ContactMessage);
            const unviewedMessages = await cMessageRepository.find({
                where: {
            ...
            await Promise.all([
                ...
                Promise.all(
                    unviewedMessages.map(m =>
                        cMessageRepository.update(m, { viewed: true }),
                    ),
                ),
            ]);
    ...
    async deleteOne(req: ServerRequest, reply: ServerReply) {
        try {
            ...
            const cMessageRepository = getRepository(ContactMessage);
            const message = await cMessageRepository.findOne(message_id, {
                relations: ['contact'],
            });
            ...
            if (message.contact.user_id !== id)
                return reply
                    .status(401)
                    .send({ message: 'Você não enviou esta mensagem!' });
            ...
            switch (target) {
                ...
                case 'bidirectional':
                    ...
                    await cMessageRepository.delete({
